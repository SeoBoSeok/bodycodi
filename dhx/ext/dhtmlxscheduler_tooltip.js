...
Scheduler.plugin(function(e) {
    !function() {
        ...
        t.tooltip.className = t.config.className,
        t.show = function(a, n) {
            if (!e.config.touch || e.config.touch_tooltip) {
                ...
                if (!this.isTooltip(_)) {
                    var o = d.x + (i.config.delta_x || 0)
                      , l = d.y - (i.config.delta_y || 0);
                    s.visibility = "hidden",
                    s.removeAttribute ? (s.removeAttribute("right"),
                    s.removeAttribute("bottom")) : (s.removeProperty("right"),
                    ...
                    document.body.offsetWidth - o - c < 0 ? (s.removeAttribute ? s.removeAttribute("left") : s.removeProperty("left"),
                    s.right = document.body.offsetWidth - o + 2 * (i.config.delta_x || 0) + "px") : 0 > o ? s.left = d.x + Math.abs(i.config.delta_x || 0) + "px" : s.left = o + "px",
                    document.body.offsetHeight - l - h < 0 ? (s.removeAttribute ? s.removeAttribute("top") : s.removeProperty("top"),
                    s.bottom = document.body.offsetHeight - l - 2 * (i.config.delta_y || 0) + "px") : 0 > l ? s.top = d.y + Math.abs(i.config.delta_y || 0) + "px" : s.top = l + "px",
                    s.visibility = "visible",
                    this.tooltip.onmouseleave = function(t) {
                        t = t || window.event;
                        for (var a = e.dhtmlXTooltip, n = t.relatedTarget; n != e._obj && n; )
                            n = n.parentNode;
        ...
        ,
        t.isTooltip = function(t) {
            var a = !1;
            for ("dhtmlXTooltip" == e._getClassName(t).split(" ")[0]; t && !a; )
                a = t.className == this.tooltip.className,
                t = t.parentNode;
            return a
        }
        ,
        ...
        e.attachEvent("onMouseMove", function(a, n) {
            var i = window.event || n
              ...
              , s = t
              , d = s.isTooltip(r)
              , _ = s.isTooltipTarget && s.isTooltipTarget(r);
            if (a || d || _) {
                var o;
                if (a || s.tooltip.event_id) {
                    ...
                    if (!l)
                        return;
                    if (s.tooltip.event_id = l.id,
                    o = e.templates.tooltip_text(l.start_date, l.end_date, l),
                    !o)
                        return s.hide()
                }
                _ && (o = "");
                ...
                if (!e.callEvent("onBeforeTooltip", [a]) || !o)
                    return;
                s.delay(s.show, s, [c || i, o])
            } else
                s.delay(s.hide, s, [], s.config.timeout_to_hide)
        }),
        ...
        e.templates.tooltip_date_format = e.date.date_to_str("%Y-%m-%d %H:%i"),
        e.templates.tooltip_text = function(t, a, n) {
            return "<b>Event:</b> " + n.text + "<br/><b>Start date:</b> " + e.templates.tooltip_date_format(t) + "<br/><b>End date:</b> " + e.templates.tooltip_date_format(a)
        }
    }()
});
