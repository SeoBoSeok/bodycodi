...
Date.prototype.format = function(format) {
    ...
    return format.replace(/(yyyy|yy|sm|mm|sd|dd|ww|sw|sh|hh|MM|ss|ap)/gi, function($1) {
...
function setCheckIn() {
	if(typeof socketAddress == "undefined") return;

	const protocol = document.location.protocol;
	const pathname = document.location.pathname;
	const address = (protocol == "https:") ? socketAddress.https : socketAddress.http;
	if(!address) return;
	...
	if(pathname.indexOf("/manager/checkin") == -1 || pathname == "/manager/entrance-member") {
...
function redirectCheckIn(data) {
    ...
    const isCheckIn = function() {
    	return (pathname.indexOf("/manager/checkin/index") > -1) ? true : false;
    };

    const isRefresh = function(seqMember) {
        if(pathname.indexOf("/manager/checkin/entrance/" + seqMember) > -1 ||
			pathname.indexOf("/manager/checkin/choose-pass/" + seqMember) > -1 ||
            pathname.indexOf("/manager/checkin/classList") > -1) return true;
        return false;
    }
	...
	switch(command) {
		case "noUser"					: if(isCheckIn()) redirectURL = "/manager/checkin/result/noUser"; break;
		case "entrance"					: if(isCheckIn()) redirectURL = "/manager/checkin/entrance/" + data.contents.SEQ_MEMBER + "/" + data.contents.INSERTID; break;
		case "exit"						: if(isCheckIn()) redirectURL = "/manager/checkin/result/exit"; break;
		case "alreadEnttranceStandby"	: if(isCheckIn()) redirectURL = "/manager/checkin/result/alreadEnttranceStandby?seqMember=" + data.contents.SEQ_MEMBER; break;
		case "refresh"					: if(isRefresh(data.contents.SEQ_PARTNER)) redirectURL = "/manager/checkin/result/individualEntrance"; break;
	}

	if(redirectURL) {
		document.location.href = redirectURL;
	} else {
		if(command == "exit" || command == "entrance" || command == "refresh") {
			if(partnerId != Number(data.contents.SEQ_PARTNER)) {
				try {
					const dataName = data.contents.NAME;
					let paramMsg = "";
					switch(command) {
						case "entrance"	: paramMsg = dataName + " 님이 입장처리를 요청하셨습니다.";
						case "exit"		: paramMsg = dataName + " 님이 퇴실처리 하셨습니다.";
						default			: if(dataName == -999) paramMsg = "-999";
					}
...
function enhancedRedirectCheckIn(data) {
	...
	if(data.result === "ok") {
		if(data.msg === "noUser") {
            ...
            //입장성공
			printerCore.requestPrint(paramUri, data.contents.SEQ_MEMBER, 2, undefined, () => {
				if(typeof doRefresh == "function") doRefresh();
			});
		}
    } else if(data.result === "entranceFailed") {
        if (data.msg === "noUser") {
            //입장실패 : 유효하지 않은 입장.
            paramMsg.enterResult = 0;
            paramMsg.playSoundNumber = 4;
