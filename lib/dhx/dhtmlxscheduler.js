...
Scheduler.getSchedulerInstance = function() {
    ...
    scheduler._click = {
        dhx_cal_data: function(e) {
            ...
            if (i && scheduler.config.select) {
                scheduler.select(i);
                var s = scheduler._getClassName(t);
    ...
    scheduler._on_mouse_move = function(e) {
        if (this._drag_mode) {
            ...
            if (this._is_pos_changed(this._drag_pos, t)) {
                ...
                var g = new Date(s - 1)
                  , v = new Date(i);
                if ("move" == this._drag_mode && scheduler.config.limit_drag_out && (+v < +scheduler._min_date || +s > +scheduler._max_date)) {
                    if (+o.start_date < +scheduler._min_date || +o.end_date > +scheduler._max_date)
                        v = new Date(o.start_date),
                        s = new Date(o.end_date);
                    else {
                        var f = s - v;
                        +v < +scheduler._min_date ? (v = new Date(scheduler._min_date),
                        t._ignores && this.config.preserve_length && this._table_view ? (v = new Date(scheduler._correct_drag_start_date(v)),
                        d._start_correction && (v = new Date(v.valueOf() + d._start_correction)),
                        s = new Date(1 * v + this._get_fictional_event_length(v, this._drag_event._event_length, d))) : s = new Date(+v + f)) : (s = new Date(scheduler._max_date),
                        ...
                        v = new Date(1 * s - this._get_fictional_event_length(s, this._drag_event._event_length, d, !0)),
                        this._ignores_detected && (v = scheduler.date.add(v, d.x_step, d.x_unit),
                        s = new Date(1 * s - this._get_fictional_event_length(s, 0, d, !0)),
                        s = scheduler.date.add(s, d.x_step, d.x_unit))) : v = new Date(+s - f))
                    }
                    var g = new Date(s - 1)
                }
                if (!this._table_view && !scheduler.config.all_timed && (!scheduler._get_section_view() && t.x != this._get_event_sday({
                    ...
                    g = new Date(s - 1),
                    "move" == this._drag_mode && (v = new Date(+s - f));
                }
    ...
    scheduler._on_mouse_down = function(e, t) {
        if (2 != e.button && !this.config.readonly && !this._drag_mode) {
            ...
            case "dhx_event_resize":
                ...
                var s = scheduler._getClassName(t);
                s.indexOf("dhx_event_resize_end") < 0 ? scheduler._drag_from_start = !0 : scheduler._drag_from_start = !1;
                break;
            case "dhx_scale_holder":
            case "dhx_scale_holder_now":
            case "dhx_month_body":
    ...
    scheduler._reset_hours_scale = function(e, t, i) {
        ...
        for (var a = new Date(1980,1,1,this.config.first_hour,0,0), n = 1 * this.config.first_hour; n < this.config.last_hour; n++) {
            ...
            r.style.height = this.config.hour_size_px + "px";
            var d = this.xy.scale_width;
            this.config.left_border && (r.className += " dhx_scale_hour_border"),
            r.style.width = d + "px",
    ...
    ,
    scheduler._render_month_scale = function(e, t, i, s) {
        function a(e) {
        ...
        s = s || Math.ceil(Math.round((n.valueOf() - i.valueOf()) / 864e5) / 7);
        for (var o = [], l = 0; 7 >= l; l++) {
        ...
        for (var g = [], l = 0; s > l; l++) {
            ...
            u.appendChild(v);
            for (var f = Math.max(a(l) - scheduler.xy.month_head_height, 0), m = 0; 7 > m; m++) {
                var p = document.createElement("td");
                v.appendChild(p);
                var x = "";
                t > i ? x = "dhx_before" : i >= n ? x = "dhx_after" : i.valueOf() == d.valueOf() && (x = "dhx_now"),
    ...
    ,
    scheduler._reset_month_scale = function(e, t, i, s) {
        var a = scheduler.date.add(t, 1, "month")
          , n = scheduler._currentDate();
        this.date.date_part(n),
        this.date.date_part(i),
        s = s || Math.ceil(Math.round((a.valueOf() - i.valueOf()) / 864e5) / 7);
        var r = Math.floor(e.clientHeight / s) - this.xy.month_head_height;
        return this._colsS.height = r + this.xy.month_head_height,
        this._colsS.heights = [],
        scheduler._render_month_scale(e, t, i, s)
    }
    ,
    ...
    scheduler._pre_render_events = function(e, t) {
        ...
        this._table_view)
            if (t)
                ...
                e.length || -1 == a[0]) {
                    var v = (r.parentNode.childNodes,
                    (a[0] + 1) * i + 1)
                      , f = v
                      , m = v + "px";
                    this.config.multi_day_height_limit && (f = Math.min(v, this.config.multi_day_height_limit),
                    m = f + "px"),
                    n.style.top = this._els.dhx_cal_navline[0].offsetHeight + this._els.dhx_cal_header[0].offsetHeight + f + "px",
                    ...
                    a[0] = 0,
                    f != v && (n.style.top = parseInt(n.style.top) + 2 + "px",
        ...
        if (!(e._sday < 0)) {
            ...
            if (s) {
                ...
                this._edit_id == e.id) {
                    ...
                    this.set_xy(f, l - 6, r - 26),
                    f.style.cssText += ";margin:2px 2px 2px 2px;overflow:hidden;",
                    g.appendChild(f),
                    this._els.dhx_cal_data[0].appendChild(g),
                    this._rendered.push(g),
                    f.innerHTML = "<textarea class='dhx_cal_editor'>" + e.text + "</textarea>",
                    this._quirks7 && (f.firstChild.style.height = r - 12 + "px"),
                    this._editor = f.firstChild,
                    ...
                    ,
                    scheduler._focus(f.firstChild, !0),
    ...
    scheduler._render_v_bar = function(e, t, i, s, a, n, r, d, o) {
        ...
        var u = e.color ? "background:" + e.color + ";" : ""
          ...
          , v = '<div event_id="' + h + '" class="' + _ + '" style="position:absolute; top:' + i + "px; left:" + t + "px; width:" + (s - 4) + "px; height:" + a + "px;" + (n || "") + '"></div>';
        l.innerHTML = v;
        var f = l.cloneNode(!0).firstChild;
        if (!o && scheduler.renderEvent(f, e, s, a, r, d))
            return f;
        f = l.firstChild;
    ...
    scheduler.render_event_bar = function(e) {
        ...
        if (n) {
            ...
            var v = e.color ? "background:" + e.color + ";" : ""
              ...
              , m = ["position:absolute", "top:" + s + "px", "left:" + a + "px", "width:" + (n - a - 15) + "px", f, v, e._text_style || ""].join(";")
    ...
    scheduler._append_drag_marker = function(e) {
        if (!e.parentNode) {
            var t = scheduler._els.dhx_cal_data[0]
              , i = t.lastChild
              , s = scheduler._getClassName(i);
            s.indexOf("dhx_scale_holder") < 0 && i.previousSibling && (i = i.previousSibling),
            s = scheduler._getClassName(i),
            i && 0 === s.indexOf("dhx_scale_holder") && i.appendChild(e)
        }
    }
    ,
    ...
    scheduler.ical = {
        ...
        parse_date: function(e, t, i) {
            ...
            var n = s[0].substr(0, 4)
              ...
              , d = s[0].substr(6, 2);
            return scheduler.config.server_utc || a ? new Date(Date.UTC(n, r, d, t, i)) : new Date(n,r,d,t,i)
        },
            ...
            if (t && i)
                ...
                default:
                    if (t.getAttribute("dhx_button"))
                        ...
                        var a, n, r;
                        -1 != i.indexOf("dhx_custom_button") && (-1 != i.indexOf("dhx_custom_button_") ? (a = t.parentNode.getAttribute("index"),
                        r = t.parentNode.parentNode) : (a = t.getAttribute("index"),
                        r = t.parentNode,
                        t = t.firstChild)),
                        a && (n = scheduler.form_blocks[scheduler.config.lightbox.sections[a].type],
                        n.button_click(a, t, r, r.nextSibling))
                    }
                }
        }
        ,
